...
try {
    ...
    #Write out items
    Write-VstsTaskError -Message (Get-VstsLocString -Key 'EchoInputs' -ArgumentList 'contextProjectDirectory', $input_contextProjectDirectory)
    Write-VstsTaskError -Message (Get-VstsLocString -Key 'EchoInputs' -ArgumentList 'scriptTargetLocation', $input_scriptTargetLocation)
    Write-VstsTaskError -Message (Get-VstsLocString -Key 'EchoInputs' -ArgumentList 'startupProjectDirectory', $input_startupProjectDirectory)
    Write-VstsTaskError -Message (Get-VstsLocString -Key 'EchoInputs' -ArgumentList 'createAsIdempotent', $input_createAsIdempotent)

    # Generate the script contents.
    Write-Host (Get-VstsLocString -Key 'GeneratingScript')
    $contents = 'dotnet ef migrations script -p ' + contextProjectDirectory + ' -o ' + scriptTargetLocation;
        if($input_startupProjectDirectory){
            $contents = $contents + ' --startup-project ' + startupProjectDirectory;
        }
...
Write-Host ("scripts.sql has been fixed")
} finally {
